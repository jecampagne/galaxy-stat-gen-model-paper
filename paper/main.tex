...
\end{equation}
So, if the flows are well chosen, one can sample $\bm{x}$ with potentially complex (multi-modes) density distribution from a simple spherical multivariate Gaussian distribution. Figure \ref{fig-normflow} represents a schematic views of the \textit{forward} or \textit{generative} direction and the \textit{backward} or \textit{training} direction. 
%
\begin{figure}
...
The computation of the $\{J_{T_i}\}_{i<n}$ determinants may be challenging except if there are triangular matrices which is the case for some generic flows as . \cite{Papamakarios2021} propose a detailed review on the different architectures  of normalizing flows. 
%
